...
const Indicator = styled.div`
  ...
  left: ${({ $active }) => {
    ...
    if ($active === "/quiz" || $active?.includes("/quiz"))
      return "calc(100% - 69px)";
    return "calc(50% - 35px)";
  }};
...
export default function QuizNavbar() {
  ...
  const navPath = pathname === "/" || pathname?.includes("/quiz");
  ...
  return (
    <Navigation>
      <NavList>
        ...
        <ListItem $active={pathname?.includes("/quiz")}>
          <StyledLink href="/quiz">
            <Icon>
              <Image
                src="/asset/gamepad.png"
                alt="quiz-logo"
                width={32}
                height={32}
              />
